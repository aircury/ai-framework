...
Before starting any non-trivial change, the agent MUST act as a routing meta-agent. Its role is to analyze the user's request and recommend the most appropriate mode — but the user makes the final decision.
...
> **Note:** Support for additional workflow frameworks (e.g. different spec formats, branching strategies, or toolchains) will be added in the future. The meta-agent routing protocol is designed to be framework-agnostic and will extend naturally to those contexts.
