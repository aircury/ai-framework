...
4. **Ensure the change is implementation-ready**
   ...
   - The change name and artifacts are specific enough that `/sdd:apply` can execute them.
...
- Prompt: "Run `/sdd:apply` to implement. When done, run `/sdd:complete` to sync `specs/features/` and clean up these ephemeral artifacts."
