...
describe('skills catalog', () => {
  it('preselects the Aircury groups by default', () => {
    expect(getDefaultSkillGroupIds('local')).toEqual(['open-spec', 'spec-kit', 'airsync', 'git']);
  });
  ...
  it('expands selected groups into installable skills', () => {
    expect(expandSkillGroups(['open-spec', 'git'], 'local').map((skill) => skill.skillName)).toEqual([
      'open-spec-propose',
      'open-spec-apply',
      'open-spec-complete',
      'open-spec-explore',
