...
export const SKILL_GROUPS: SkillGroup[] = [
  {
    id: 'open-spec',
    label: 'OpenSpec',
    description: 'Structured propose/apply/complete workflow for complex changes',
    kind: 'aircury',
  ...
  },
  {
    id: 'spec-kit',
    label: 'Spec Kit',
    description: 'Formal specification workflow for feature definition and delivery',
    kind: 'aircury',
    defaultSelected: true,
    scopes: ['local', 'global'],
  },
...
export function getSkillGroups(scope: SkillScope, kind?: SkillGroupKind): SkillGroup[] {
...
export function getDefaultSkillGroupIds(scope: SkillScope): string[] {
  return getSkillGroups(scope).filter((group) => group.defaultSelected).map((group) => group.id);
}

export function expandSkillGroups(groupIds: string[], scope: SkillScope): SkillDefinition[] {
