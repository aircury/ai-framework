...
const INSTALL_CONTENT_SECTION_MARKER = /<!-- FILE: ([^>]+) -->/g;

function getInstallContentByPath(path: string): string {
  const matches = [...installContentSource.matchAll(INSTALL_CONTENT_SECTION_MARKER)];
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    const end = matches[index + 1]?.index ?? installContentSource.length;
    return `${installContentSource.slice(start, end).trim()}\n`;
  }

  throw new Error(`Missing install content for ${path}`);
}
...
function getSpecsFiles(moduleIds: StandardModuleId[]): InstallFile[] {
  const files: InstallFile[] = [
    {
      ...
      content: getInstallContentByPath("specs/features/README.md"),
  ...
  if (moduleIds.includes("decision-records")) {
    files.push({
      ...
      content: getInstallContentByPath("specs/decisions/README.md"),
  ...
  if (moduleIds.includes("frontend")) {
    files.push({
      ...
      content: getInstallContentByPath("specs/ui/README.md"),
      description: "Frontend design system starter guide",
    });
    files.push({
      path: "specs/ui/frontend-workflow.md",
      content: getInstallContentByPath("specs/ui/frontend-workflow.md"),
